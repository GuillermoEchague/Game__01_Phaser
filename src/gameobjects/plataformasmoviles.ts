...
export default class PlataformasMoviles extends Phaser.Physics.Arcade.Group {
  ...
  constructor(
    ...
    let nombreObjetoPlataforma: string = this.horizontal
      ? constants.MAPAS.PLATAFORMAHORIZONTAL
      : constants.MAPAS.PLATAFORMAVERTICAL;

    // añade los objetos de los enemigos desde el array de sprites obtenidos del mapa al grupo
    this.addMultiple(
      this.escena.mapaNivel.createFromObjects(nombreObjeto, {
        name: nombreObjetoPlataforma,
        key: idObjeto,
      })
    );
    ...
    this.children.entries.map((plataforma: any) => {
      ...
      if (this.horizontal) {
        ...
        this.muevePlataformaHorizontal(
          Phaser.Math.Between(0, 1) ? "izda" : "dcha",
          plataforma
        );
      } else {
        plataforma.body.setFrictionY(1);
        plataforma.body.setVelocityY(this.velocidad);
        this.muevePlataformaVertical(
          Phaser.Math.Between(0, 1) ? "arriba" : "abajo",
          plataforma
        );
      }
    });
